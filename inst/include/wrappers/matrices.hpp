...
inline Eigen::Matrix<T, Dynamic, Dynamic> dblint_matrix_to_Matrix_(const U& x) {
  ...
  if (std::is_same<U, doubles_matrix<>>::value) {
    Eigen::Map<Eigen::Matrix<T, Dynamic, Dynamic>> y(reinterpret_cast<T*>(REAL(x.data())),
                                                     n, m);
    return y;
  } else {
    Eigen::Map<Eigen::Matrix<T, Dynamic, Dynamic>> y(
...
inline integers_matrix<> as_integers_matrix(
    const Eigen::Matrix<int, Dynamic, Dynamic>& A) {
