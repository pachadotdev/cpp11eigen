...
inline Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> dblint_matrix_to_Matrix_(const U& x) {
  const int n = x.nrow();
  const int m = x.ncol();

  if (std::is_same<U, doubles_matrix<>>::value) {
    Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>> y(reinterpret_cast<T*>(REAL(x.data())),
                                                     n, m);
    return y;
  } else {
...
inline Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> as_Matrix(const doubles_matrix<>& x) {
  return dblint_matrix_to_Matrix_<double, doubles_matrix<>>(x);
}

inline Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic> as_Matrix(const integers_matrix<>& x) {
...
template <typename T, typename U>
inline U Matrix_to_dblint_matrix_(const Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& A) {
  const int n = A.rows();
  const int m = A.cols();
